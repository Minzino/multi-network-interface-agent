...
func (l *EnvironmentConfigLoader) Load() (*Config, error) {
    config := &Config{
        ...
        Agent: AgentConfig{
            PollInterval:       getEnvDurationOrDefault("POLL_INTERVAL", constants.DefaultPollInterval*time.Second),
            MaxRetries:         getEnvIntOrDefault("MAX_RETRIES", constants.DefaultMaxRetries),
            RetryDelay:         getEnvDurationOrDefault("RETRY_DELAY", constants.DefaultRetryDelay*time.Second),
            CommandTimeout:     getEnvDurationOrDefault("COMMAND_TIMEOUT", constants.DefaultCommandTimeout*time.Second),
            BackupDirectory:    getEnvOrDefault("BACKUP_DIR", constants.DefaultBackupDir),
            MaxConcurrentTasks: getEnvIntOrDefault("MAX_CONCURRENT_TASKS", constants.DefaultMaxConcurrentTasks),
            Backoff: BackoffConfig{
                Enabled:     getEnvBoolOrDefault("BACKOFF_ENABLED", true),
                MaxInterval: getEnvDurationOrDefault("BACKOFF_MAX_INTERVAL", getEnvDurationOrDefault("POLL_INTERVAL", constants.DefaultPollInterval*time.Second)*10),
                Multiplier:  getEnvFloatOrDefault("BACKOFF_MULTIPLIER", constants.DefaultBackoffMultiplier),
            },
